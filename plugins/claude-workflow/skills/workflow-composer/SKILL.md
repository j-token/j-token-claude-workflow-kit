...
description: 요구사항 발견, 디버깅, UI/Figma 구현이 한 요청에 섞여 있을 때 사용합니다. 단일 분류를 강제하지 않고 여러 워크플로우 모듈을 조합하며, 레포 내 `./.claude/temp`에 작업 문서를 남기고 질문 순서를 조율합니다.
---
...
사용자가 다른 위치를 명시하지 않으면 레포 루트의 `./.claude/temp` 디렉터리에 작업 문서를 작성합니다.

`./.claude/temp` 디렉터리가 없으면 생성합니다. OS 시스템 임시 디렉터리(`/tmp`, `$TMPDIR`, `%TEMP%`)에는 작성하지 않습니다.

파일명은 다음 형식을 사용합니다.

```text
./.claude/temp/YYYYMMDD-HHMM-<type>-<topic>-workflow.md
```
...
작업 문서를 `./.claude/temp`에 작성한 뒤에는 사용자의 명시적인 허가가 있어야 실제 작업(코드 수정, 구현, 디버깅 등)을 시작할 수 있습니다.

- 문서 작성까지만 진행하고, 사용자가 "시작해", "진행해", "구현해" 같이 명시적으로 허가하기 전에는 작업을 시작하지 않습니다.
- 사용자가 다음 단계를 말하지 않으면 허가를 기다린다는 것을 알리고 멈춥니다.

## 구현 진입 규칙 (plan 모드)

작업 시작 게이트를 통과해 "구현 지시 대기" 단계에 들어가면, 곧바로 코드를 수정하지 않고 Claude Code의 plan 모드로 전환합니다.
