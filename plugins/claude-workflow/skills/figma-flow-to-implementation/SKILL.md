...
6. `./.claude/temp` 아래 임시 UI 작업 문서를 갱신합니다.
7. 사용자가 요청하거나 확정한 뒤에만 최종 단일 UI 스펙/구현 문서를 작성합니다.
8. 문서를 작성한 뒤에는 사용자의 명시적인 허가가 있어야 작업을 시작합니다.
9. 허가를 받아 구현 지시 대기 단계가 되면 Claude Code의 plan 모드로 전환해 확정 문서 기준 구현 계획을 제시합니다.
10. 사용자가 plan을 승인한 뒤에만 plan 모드를 빠져나와 구현합니다.
...
```text
./.claude/temp/YYYYMMDD-HHMM-feat-<topic>-ui-workflow.md
```
