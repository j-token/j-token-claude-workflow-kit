...
description: 의도하지 않은 브랜치나 보호 브랜치에 push하는 사고를 막기 위한 안전 워크플로우입니다. Claude가 git push 실행, 준비, 검토, 설명, force push, upstream 설정, 브랜치 게시, PR 준비를 요청받았을 때 사용합니다. 사용자가 매번 명시적으로 push를 요청하거나 승인하지 않으면 push는 금지됩니다.
---
...
사용자가 Claude에게 push를 요청하면 실행 전에 확인한 로컬 브랜치, upstream 상태, 대상 refspec, force push 여부를 보여줍니다.
