# Claude 워크플로우 플러그인 설계

## TL;DR

이 문서는 사용자의 반복 작업 3가지를 Claude 플러그인으로 구현하기 전 정리한 설계 초안이다.
...
사용자가 매번 같은 방식으로 설명하고, Claude가 매번 다른 방식으로 추측하는 문제를 줄인다.
...
| `workflow-doc-state` | 각 플로우의 합의 상태를 OS 시스템 임시 디렉터리의 `claude-workflow/` 하위 문서에 남긴다. 사용자가 명시한 경우에만 레포 본문에 작성한다. |
...
- 구현은 문서가 확정된 뒤 시작한다. 문서가 확정되어 "구현 지시 대기" 상태에 들어가면 Claude Code의 plan 모드로 전환해 구현 계획을 제시하고, 사용자가 plan을 승인한 뒤에만 구현한다.
- 문서에는 채택한 결정뿐 아니라 버린 선택지도 짧게 남긴다.
- 기본 문서 저장 위치는 OS 시스템 임시 디렉터리의 `claude-workflow/` 하위 폴더다.
...
사용자는 처음부터 완성된 요구사항을 주지 않는다. 배경과 원하는 방향을 먼저 말하고, Claude가 조사와 제안을 가져오면 사용자가 맞는 부분과 아닌 부분을 피드백한다.

이 플로우의 핵심은 Claude가 곧바로 스펙을 확정하지 않고, 대화 중 생기는 질문과 조사 결과를 문서의 결정 사항으로 축적하는 것이다.
...
| 문서 | 목적 |
|---|---|
| `<system-temp>/claude-workflow/YYYYMMDD-HHMM-feat-<topic>-workflow.md` | 대화 중 나온 요구사항, 질문, 조사 결과, 결정 사항, 구현 계획을 함께 축적한다. |
...
```mermaid
flowchart TD
    A["사용자: 배경/원하는 것 설명"] --> B["Claude: 현재 이해 요약"]
    B --> C["Claude: 조사 질문/확인 질문 분리"]
    C --> D["Claude: 코드/문서/외부 자료 조사"]
    D --> E["Claude: 구현 방향 제안"]
    E --> F["사용자: 맞음/아님/추가 질문 피드백"]
    F --> G["Claude: <system-temp>/claude-workflow/YYYYMMDD-HHMM-feat-<topic>-workflow.md 갱신"]
    G --> H{"스펙 작성 요청?"}
    H -- "아니오" --> C
    H -- "예" --> I["Claude: 단일 스펙/구현 문서 작성"]
    I --> J["사용자: 문서 수정 피드백"]
    J --> K{"문서 확정?"}
    K -- "아니오" --> I
    K -- "예" --> L["사용자: 이 문서 구현해줘"]
...
사용자는 증상을 대충 말하거나 스크린샷만 줄 수 있다. 이때 Claude는 즉시 원인을 확정하지 않고, 어떤 행동 중 발생했는지, 언제 발생했는지, 어떤 플랫폼에서 확인했는지 물어야 한다.
...
| 문서 | 목적 |
|---|---|
| `<system-temp>/claude-workflow/YYYYMMDD-HHMM-bug-<topic>-workflow.md` | 버그 리포트, 사용자 정정, 원인 후보, 수정 계획, 검증 결과를 한 파일에 축적한다. |
...
```mermaid
flowchart TD
    A["사용자: 증상/스크린샷/로그 전달"] --> B["Claude: 사실과 추정 분리"]
    B --> C["Claude: 재현 조건 질문"]
    C --> D["사용자: 행동/시점/플랫폼 피드백 또는 재현 행동 수행"]
    D --> E["Claude: bug-report 작성"]
    E --> F["사용자: '나 이러진 않았어' 등 정정"]
    F --> G["Claude: bug-report 수정"]
    G --> H{"사용자: 수정 디버깅 시작 지시?"}
    H -- "아니오" --> C
    H -- "예" --> I["Claude: 재현/로그/코드 조사"]
    I --> J["Claude: 원인 후보 좁히기"]
    J --> K["Claude: 수정"]
    K --> L["Claude: 고쳐진 게 맞는지 검증"]
    L --> M["Claude: 결과를 <system-temp>/claude-workflow/YYYYMMDD-HHMM-bug-<topic>-workflow.md에 반영"]
```
...
따라서 Claude는 제공된 시각 자료를 먼저 읽고, 화면의 의미와 전이 흐름을 추론한 뒤 Mermaid로 사용자에게 확인받아야 한다. Figma 플러그인을 우선 사용할 수 있지만 필수는 아니다. 링크, 스크린샷, 설명 기반 경로도 선택 가능하다.
...
| 문서 | 목적 |
|---|---|
| `<system-temp>/claude-workflow/YYYYMMDD-HHMM-feat-<topic>-ui-workflow.md` | Figma 화면 목록, 화면 역할, 추정 전이, 사용자 피드백, 구현 계획을 한 파일에 축적한다. |
...
```mermaid
flowchart TD
    A["사용자: Figma 링크 전달"] --> B["Claude: 모든 링크 읽기"]
    B --> C["Claude: 화면 이름/역할 추출"]
    C --> D["Claude: 추정 화면 전이 작성"]
    D --> E["Claude: Mermaid 플로우 제시"]
    E --> F["사용자: 전이/요소/조건 피드백"]
    F --> G["Claude: <system-temp>/claude-workflow/YYYYMMDD-HHMM-feat-<topic>-ui-workflow.md 갱신"]
    G --> H{"흐름 확정?"}
    H -- "아니오" --> E
    H -- "예" --> I["Claude: 단일 UI 스펙/구현 문서 작성"]
    I --> J["사용자: 구현 방식 피드백"]
    J --> K{"문서 확정?"}
    K -- "아니오" --> I
    K -- "예" --> L["사용자: 이 문서 구현해줘"]
...
초기 버전은 MCP 서버 없이 Claude 스킬로 구현한다.
...
반복 피드백을 안정적으로 반영하기 위해 OS 시스템 임시 디렉터리의 `claude-workflow/` 하위 폴더에 상태 문서를 저장한다. 사용자가 정식 문서 위치를 따로 지시한 경우에만 레포 본문에 작성한다.

```text
<system-temp>/claude-workflow/
...
- 임시 문서는 기본적으로 OS 시스템 임시 디렉터리의 `claude-workflow/` 하위 폴더에 작성한다.
...
- 시스템 임시 디렉터리 아래 `claude-workflow/` 폴더가 없으면 생성해도 된다.
